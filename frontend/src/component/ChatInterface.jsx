      ...
      <div className="flex flex-col h-screen max-w-7xl mx-auto p-4 font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-br from-gray-800 via-gray-700 to-gray-600 rounded-lg shadow-inner ">
          {messages.map((message) => (
            ...
            >
              <div
                className={`max-w-[70%] rounded-xl p-4 ${message.sender === "user"
                    ? "bg-indigo-500 text-white"
                    : "bg-gray-700 text-gray-200 shadow-lg"
                  }`}
                style={{
                  whiteSpace: "pre-wrap", // Ensures newlines and spaces are preserved
                  textAlign: message.sender === "user" ? "right" : "left", // Aligns text appropriately
                }}
              >
                {message.text}
              </div>
        ...
        >
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`bg-indigo-600 text-white p-3 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500 ${isLoading ? "opacity-50 cursor-not-allowed" : ""
                }`}
            >
